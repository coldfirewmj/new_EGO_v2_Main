...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    broadcast_odom_sub      = nh.subscribe<nav_msgs::Odometry>("planning/broadcast_odom_recv",100,
                                                                  &EGOReplanFSM::RecvBroadcastOdomCallback,
                                                                  this);
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case WAIT_TARGET:
    {
      if (has_start)
      {
      cout<<"drone is is:"<<planner_manager_->pp_.drone_id<<endl;
      cout<<"the all averange time is:"<<planner_manager_->total_plan_time/planner_manager_->total_plan_num<<endl;
      cout<<"the all jerk time is:"<<planner_manager_->totaljerk/ planner_manager_->total_num<<endl;
      cout<<"the all speed time is:"<<planner_manager_->totalspeed/ planner_manager_->total_num/planner_manager_->pp_.max_vel_*100<<"%"<<endl;
      has_start=false;
      }
    ...
    case EXEC_TRAJ:
    {
      ...
      double jerk=planner_manager_->traj_.local_traj.traj.getJer(t_cur).norm();
      double speed=planner_manager_->traj_.local_traj.traj.getVel(t_cur).norm();
      planner_manager_->totaljerk+=jerk*jerk;
      planner_manager_->totalspeed+=speed;
      planner_manager_->total_num++;
  ...
  void EGOReplanFSM::RecvBroadcastOdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
  {
    std::string dronname=msg->child_frame_id;
    if (planner_manager_->all_Odom.count(dronname))
    {
      planner_manager_->all_Odom[dronname]=*msg;
    }
    else
    {
      planner_manager_->all_Odom.insert(std::pair<std::string,nav_msgs::Odometry>(dronname,*msg));
    }
    //std::cout<<planner_manager_->all_Odom["drone_0"]<<planner_manager_->all_Odom["drone_1"]<<std::endl;
  }
