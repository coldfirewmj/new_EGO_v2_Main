...
namespace ego_planner
{
                                         ...
                                         double &costp)
  {
    ...
    std::unordered_set<int> id_have_traj;
    for (size_t id = 0; id < swarm_trajs_->size(); id++)
    {
      id_have_traj.insert(swarm_trajs_->at(id).drone_id);
    ...
    for (auto iter=swarm_odom_->begin();iter!=swarm_odom_->end();iter++)
    {
      std::string local_dronename="drone_"+std::to_string(drone_id_);
      if (local_dronename==iter->first)
      {//iter指向的odom为当前无人机,不能包含在内
        continue;
      }
      //删除local_dronename的“drone_”
      local_dronename=iter->first.substr(6,iter->first.length());
      std::istringstream ss(local_dronename);
      size_t id ;
      ss>>id;
      if (id_have_traj.count(id))
      {//有traj的无人机需要删除以避免重复计算
        continue;
      }
      Eigen::Vector3d swarm_p, swarm_v;
      swarm_p(0)=iter->second.pose.pose.position.x;
      swarm_p(1)=iter->second.pose.pose.position.y;
      swarm_p(2)=iter->second.pose.pose.position.z;
      swarm_v(0)=iter->second.twist.twist.linear.x;
      swarm_v(1)=iter->second.twist.twist.linear.y;
      swarm_v(2)=iter->second.twist.twist.linear.z;
      //dist_vec为两者之间的相对距离
      Eigen::Vector3d dist_vec = p - swarm_p;
      const double CLEARANCE = (swarm_clearance_ + swarm_clearance_) * 1.5; // 1.5 is to compensate slight constraint violation
      const double CLEARANCE2 = CLEARANCE * CLEARANCE;
      double ellip_dist2 = dist_vec(2) * dist_vec(2) * inv_a2 + (dist_vec(0) * dist_vec(0) + dist_vec(1) * dist_vec(1)) * inv_b2;
      double dist2_err = CLEARANCE2 - ellip_dist2;
      double dist2_err2 = dist2_err * dist2_err;
  ...
  void PolyTrajOptimizer::setSwarmOdoms(SwarmOdomData *swarm_odom_ptr)  { swarm_odom_=swarm_odom_ptr;}
